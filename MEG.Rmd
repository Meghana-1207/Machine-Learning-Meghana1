---
title: "Machine Learning Assignment - Meghna Ramireddy"
output:
...
###importing data
```{r}
BankData <- read.csv("~/Downloads/UniversalBank (1).csv", stringsAsFactors=TRUE)
 
```

```{r}
#Eliminating variables [id & zip code] from the dataset
df=subset(BankData, select=-c(ID, ZIP.Code ))
```
...
BankData <- cbind(df_without_education, dummy_Education)              #main dataset
```
...
Train_Data      = BankData[Train_Index,]  #3001 observations

Validation_Data = BankData[-Train_Index,] #1999 observations
```
...
maindata.norm.df[,-7] <- predict(norm.values,BankData[,-7]) #Training + Validation data
...
Test_Index_1 = createDataPartition(BankData$Age, p= 0.2 , list=FALSE) #20% test data 
Test_Data_1  = BankData [Test_Index_1,]

Rem_DATA = BankData[-Test_Index_1,] #80% remaining data [training + validation]
